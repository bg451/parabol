...
void net_set_fd(int fd, net_fd_type type, void (*data_callback)(), void (*sent_callback)(), unsigned int alloc_buf)
{
  ...
  net_fds[fd].ctx = kbuf_new_ctx();
  if (alloc_buf == 1)
  {
    net_fds[fd].readbuf = kbuf_init(net_fds[fd].ctx, 0);
    net_fds[fd].linebuf = kbuf_init(net_fds[fd].ctx, 0);
    net_fds[fd].sendbuf = kbuf_init(net_fds[fd].ctx, 0);
    net_fds[fd].peerbuf = kbuf_init(net_fds[fd].ctx, 0);
    net_fds[fd].sockbuf = kbuf_init(net_fds[fd].ctx, 0);
  } else
  {
  ...
  switch (type)
  {
    case NET_FD_LISTEN: case NET_FD_READ: case NET_FD_READLINE: case NET_FD_RAW:
      if (asio_add_fd(fd, ASIO_R) != 0) { kperror("asio_add_fd()"); exit(1); }
      break;
    case NET_FD_SEND:
      if (asio_add_fd(fd, ASIO_W) != 0) { kperror("asio_add_fd()"); exit(1); }
      break;
  }
}
...
void net_send_buf(int fd, kbuf *buf)
{
  Kassert(FD_VALID(fd));
  kbuf_append_buf(net_fds[fd].sendbuf, buf);
}
void net_unset_fd(int fd)
{
  ...
  {
    kbuf_free_ctx(net_fds[fd].ctx);
    net_fds[fd].ctx = (kbuf_ctxh)0;
...
void net_start_listen(void)
{
  ...
  if ((sock = socket(AF_INET, SOCK_STREAM, 0)) < 0 || sock >= ASIO_MAX_FDS) { kperror("socket()"); exit(1); }
  i = 1;
  if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (char *)&i, sizeof(i)) != 0) { kperror("setsockopt()"); exit(1); }
  memset(&sin, 0, sizeof(sin));
  sin.sin_family = AF_INET;
  sin.sin_addr.s_addr = cfg.listen_addr.s_addr;
  sin.sin_port = htons(cfg.listen_port);
  if (bind(sock, (struct sockaddr *)&sin, sizeof(sin)) != 0) { kperror("bind()"); exit(1); }
  if (listen(sock, LISTEN_BACKLOG) != 0) { kperror("listen() (decrease LISTEN_BACKLOG?)"); exit(1); }
  net_set_fd(sock, NET_FD_LISTEN, NULL, NULL, 0);
  klogf(LOG_INFO, "Listening on port %d.", cfg.listen_port);
}
void test_readline(int fd, net_fd_entry *net_ent, kbuf *line)
{
  kbuf_asciiz(line);
...
void net_accept_connection(int fd)
{
  ...
  sin_len = sizeof(sin);
  memset(&sin, 0, sizeof(sin));
  if ((newfd = accept(fd, (struct sockaddr *)&sin, &sin_len)) < 0) { kperror("accept()"); return; }
  if (!FD_VALID(newfd))
  {
    klogf(LOG_INFO, "Client limit reached. Increase ASIO_MAX_FDS?");
    close(newfd);
    return;
  }
  net_set_fd(newfd, NET_FD_INIT_TYPE, net_fd_init_data_callback, net_fd_init_sent_callback, 1);
  kbuf_sprintf(net_fds[newfd].peerbuf, "%s:%u", inet_ntoa(sin.sin_addr), ntohs(sin.sin_port));
  sin_len = sizeof(sin);
  memset(&sin, 0, sizeof(sin));
  if (getsockname(newfd, (struct sockaddr *)&sin, &sin_len) == 0)
  {
    kbuf_sprintf(net_fds[newfd].sockbuf, "%s:%u", inet_ntoa(sin.sin_addr), ntohs(sin.sin_port));
  } else
  {
    kbuf_strcpy(net_fds[newfd].sockbuf, "");
  }
  klogf(LOG_CONNECTION, "%d: Connection from %s", newfd, kbuf_data(net_fds[newfd].peerbuf));
}
...
void net_handle_send(int fd)
{
  ...
  if (!kbuf_empty(net_fds[fd].sendbuf))
  {
    if ((n = write(fd, kbuf_data(net_fds[fd].sendbuf), kbuf_idx(net_fds[fd].sendbuf))) <= 0)
    {
      if (errno != EINTR && errno != EAGAIN) net_unset_fd(fd);
      return;
    }
    kbuf_consume(net_fds[fd].sendbuf, n);
    if (kbuf_empty(net_fds[fd].sendbuf) && net_fds[fd].send_fd == -1)
    {
      net_sent(fd);
      return;
    }
  } else if (net_fds[fd].send_fd != -1)
  {
    ...
    {
      if (errno != EAGAIN && errno != EINTR) { kperror("sendfile()"); net_unset_fd(fd); }
      return;
    }
    ...
    {
      if (lseek(fd, net_fds[fd].send_fd_off, SEEK_SET) == (off_t)-1) kperror("lseek()"); 
    ...
    if (m < n) kbuf_append_data(net_fds[fd].sendbuf, &readbuf[m], n - m);
#endif
  } else
  {
    net_sent(fd);
...
void net_handle_readline(int fd)
{
  ...
  kbuf_append_data(net_fds[fd].readbuf, readbuf, n);
  if (kbuf_idx(net_fds[fd].readbuf) > MAX_LINE_LEN)
  {
    net_unset_fd(fd);
    return;
  }
  while ((i = kbuf_chr(net_fds[fd].readbuf, '\n')) >= 0 && net_fds[fd].type == NET_FD_READLINE)
  {
    kbuf_split(net_fds[fd].readbuf, net_fds[fd].linebuf, i);
    if ((j = kbuf_chr(net_fds[fd].linebuf, '\r')) >= 0) kbuf_set_idx(net_fds[fd].linebuf, j);
    if (net_fds[fd].data_callback != NULL) net_fds[fd].data_callback(fd, &net_fds[fd], net_fds[fd].linebuf);
  }
  if (net_fds[fd].type == NET_FD_READ && kbuf_idx(net_fds[fd].readbuf) > 0)
  {
    ...
    kbuf_set_idx(net_fds[fd].readbuf, 0);
  }
}
void net_handle_read(int fd)
{
  ...
  DEBUGF("read %d bytes", n);
  kbuf_set_data(net_fds[fd].readbuf, readbuf, n);
  if (net_fds[fd].data_callback != NULL) net_fds[fd].data_callback(fd, &net_fds[fd], net_fds[fd].readbuf);
}
