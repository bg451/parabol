...
#ifdef ASIO_USE_EPOLL
int epoll_fd;
#ifdef ASIO_USE_KEPOLL
#include "Kepoll.h"
#else
#define Kepoll_create epoll_create
#define Kepoll_ctl epoll_ctl
#define Kepoll_wait epoll_wait
#endif
#endif
...
#endif
#ifdef ASIO_USE_EPOLL
  if ((epoll_fd = Kepoll_create(ASIO_MAX_FDS)) == -1) { perror("epoll_create()"); exit(1); }
#endif
#ifdef ASIO_USE_POLL
  num_pollfds = 0;
#endif
...
int asio_add_fd(int fd, asio_event_type events)
{
  ...
  if (Kepoll_ctl(epoll_fd, EPOLL_CTL_ADD, fd, &ev) != 0) { perror("epoll_ctl()"); return -1; }
#endif
#ifdef ASIO_USE_POLL
...
int asio_set_events(int fd, asio_event_type events)
{
  ...
  if (events & ASIO_R) ev.events |= EPOLLIN | EPOLLPRI | EPOLLERR | EPOLLHUP;
  if (events & ASIO_W) ev.events |= EPOLLOUT | EPOLLERR | EPOLLHUP;
  if (Kepoll_ctl(epoll_fd, EPOLL_CTL_MOD, fd, &ev) != 0) { perror("epoll_ctl()"); return -1; }
  return 0;
#endif
#ifdef ASIO_USE_POLL
...
int asio_del_fd(int fd, asio_event_type events)
{
  ...
  Kepoll_ctl(epoll_fd, EPOLL_CTL_DEL, fd, &ev);
#endif
#ifdef ASIO_USE_POLL
  for (i = 0; i < num_pollfds; i ++) if (pollfds[i].fd == fd) break;
  if (i == num_pollfds) return - 1;
...
#endif
#ifdef ASIO_USE_EPOLL
  if ((j = Kepoll_wait(epoll_fd, epoll_events, ASIO_MAX_FDS, 1000)) < 0)
  {
    return &ret; /*XXX*/
  }
  if (j > 0)
