...
struct parabol_config_token
{
  unsigned char *key;
  unsigned int type;
  void *val;
} parabol_config_tokens[] =
{
...
#define MAX_CFG_TOKENS 16
  ...
  while (fgets(readbuf, sizeof(readbuf) - 1, f) != NULL)
  {
    ...
    for (i = 0; parabol_config_tokens[i].key != NULL; i ++) if (strcasecmp(parabol_config_tokens[i].key, tokenv[0]) == 0) break;
    if (parabol_config_tokens[i].key == NULL) { CFG_ERR_HDR(); fprintf(stderr, "Unknown key '%s'\n", tokenv[0]); return -1; }
    if (parabol_config_tokens[i].type == CFG_BOOL)
    {
      if (tokenc != 1) { CFG_ERR_HDR(); fprintf(stderr, "Invalid number of arguments (expected [!]%s)\n", tokenv[0]); return -1; }
      DEBUGF("bool value '%s' = %u", parabol_config_tokens[i].key, bool_val);
      *(int *)parabol_config_tokens[i].val = bool_val;
    } else if (bool_val != 0) switch (parabol_config_tokens[i].type)
    {
      case CFG_NONE: break;
      case CFG_INT:
      {
        if (tokenc != 2) { CFG_ERR_HDR(); fprintf(stderr, "Invalid number of arguments (expected %s <number>)\n", tokenv[0]); return -1; }
        *(int *)parabol_config_tokens[i].val = atoi(tokenv[1]);
        break;
      }
      case CFG_STR:
      {
        ...
        dest = parabol_config_tokens[i].val;
      ...
      case CFG_TABLE:
      {
        ...
        dest = parabol_config_tokens[i].val;
        tbl = (kbuf_table *)*dest;
        kbuf_table_entry_add_str(cfg_ctx, tbl, tokenv[1], tokenv[2]);
        break;
      }
      case CFG_REGEX_TABLE:
      {
        ...
        compbuf = kbuf_init(cfg_ctx, sizeof(regex_t));
        if ((ret = regcomp((regex_t *)kbuf_data(compbuf), tokenv[1], REG_EXTENDED)) != 0)
        {
          unsigned char errbuf[BUF_SIZE];
          
          regerror(ret, (regex_t *)kbuf_data(compbuf), errbuf, sizeof(errbuf));
          fprintf(stderr, "Parsing of regex '%s' failed: %s\n", tokenv[1], errbuf);
          return -1;
        }
        kbuf_set_idx(compbuf, sizeof(regex_t));
        valbuf = kbuf_init(cfg_ctx, 0);
        kbuf_strcpy(valbuf, tokenv[2]);
        dest = parabol_config_tokens[i].val;
        tbl = (kbuf_table *)*dest;
        kbuf_table_entry_add_buf(cfg_ctx, tbl, compbuf, valbuf);
        kbuf_free(cfg_ctx, valbuf);
        break;
      }
      case CFG_INADDR:
      {
        ...
        addr = parabol_config_tokens[i].val;
      ...
      {
        int (*func)();
        func = parabol_config_tokens[i].val;
...
void cfg_init(void)
{
  ...
  cfg.vhosts = kbuf_table_init(cfg_ctx, KBUF_TABLE_NOCASE);
  cfg.mime_types = kbuf_table_init(cfg_ctx, KBUF_TABLE_NOCASE);
  cfg.rewrite_rules = kbuf_table_init(cfg_ctx, KBUF_TABLE_BIN);
  cfg.allow_clients = kbuf_table_init(cfg_ctx, KBUF_TABLE_BIN);
  cfg.deny_clients = kbuf_table_init(cfg_ctx, KBUF_TABLE_BIN);
